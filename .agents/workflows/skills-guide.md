---
description: read this whenever you need to read or initiate any skill, this guides you through all the skills and workflows available
---
...
2. **Skills** — Load the ENTIRE skill folder (see Full Context Loading Protocol below)
3. **Skill+Workflow pairs** — Workflow routes you to the skill folder; load ALL files there

## Full Context Loading Protocol (MANDATORY)

When ANY skill is activated, you MUST load ALL files in the skill folder. No exceptions.

### Loading Sequence

1. `list_dir` on `.agents/skills/<skill-name>/` to discover all contents
2. `view_file` on `SKILL.md` — the entry point
3. `view_file` on EVERY file in `references/` directory
4. `view_file` on EVERY file in `scripts/` directory
5. `view_file` on EVERY file in any other subdirectory (evals/, examples/, agents/, assets/)
6. Only THEN proceed with the skill's instructions

### Why Full Loading?

- SKILL.md is an overview/router. The references/ and scripts/ contain the REAL knowledge
- Partial loading → hallucinated procedures, missed best practices
- "On-demand" loading is DEPRECATED — agents never decided they "needed" the files and systematically skipped them
- Reading only SKILL.md = operating at ~20% capability
