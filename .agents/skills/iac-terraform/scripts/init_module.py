...
Examples:
  ...
  %(prog)s my-vpc --json
    ...
    if not args.module_name.replace("-", "").replace("_", "").isalnum():
        print(
            "Error: Module name should only contain lowercase letters, numbers, hyphens, and underscores",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.json:
        ...
        if result["success"]:
            ...
            print("   2. Update main.tf with your resources")
            print("   3. Run: terraform init")
            print("   4. Run: terraform validate")
            print("   5. Generate docs: terraform-docs markdown . > README.md")
        else:
