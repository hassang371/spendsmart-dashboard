...
class PipelineAnalyzer:
                ...
                for step in steps:
                    if isinstance(step, dict):
                        ...
                        if "pip install" in run_cmd:
                            has_pip_cache = any(
                                s.get("uses", "").startswith("actions/cache")
    ...
    def _check_gitlab_specific_features(self, config: Dict):
        ...
        has_needs = any(isinstance(v, dict) and "needs" in v for v in config.values())
...
def main():
    ...
    # Create analyzer
    analyzer = PipelineAnalyzer(platform=args.platform, repo=args.repo)
