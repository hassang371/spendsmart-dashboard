...
class ArgoCDHealthChecker:
    ...
    def check_application_health(self, app: Dict) -> Dict[str, Any]:
        ...
        if unhealthy_resources:
            ...
            result["recommendations"].append(
                f"kubectl get {unhealthy_resources[0]['kind']} -n {app['spec']['destination']['namespace']}"
            )
        ...
        if has_label_tracking:
            result["recommendations"].append(
                "⚠️  Using legacy label-based tracking. Consider migrating to annotation-based tracking (ArgoCD 3.x default)"
            )
    ...
    def print_summary(self, results: List[Dict]):
        ...
        if tabulate:
            ...
            for r in results:
                status_icon = "❌" if r["issues"] else "✅"
                print(
                    f"{status_icon} {r['name']}: Health={r['health_status']}, Sync={r['sync_status']}, Issues={len(r['issues'])}"
                )
