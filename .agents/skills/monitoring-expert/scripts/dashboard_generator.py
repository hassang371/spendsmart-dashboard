...
class DashboardGenerator:
    ...
    def generate_webapp_dashboard(self, service: str):
        ...
        self.add_variable(
            "service",
            "Service",
            f"label_values({service}_http_requests_total, service)",
        )
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f"sum(rate({service}_http_requests_total[5m])) by (status)",
                    "legend": "{{status}}",
                }
            ],
            unit="reqps",
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f"histogram_quantile(0.50, sum(rate({service}_http_request_duration_seconds_bucket[5m])) by (le))",
                    "legend": "p50",
                },
                {
                    "query": f"histogram_quantile(0.95, sum(rate({service}_http_request_duration_seconds_bucket[5m])) by (le))",
                    "legend": "p95",
                },
                {
                    "query": f"histogram_quantile(0.99, sum(rate({service}_http_request_duration_seconds_bucket[5m])) by (le))",
                    "legend": "p99",
                },
            ],
            unit="s",
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'sum(rate({service}_http_requests_total{{status=~"5.."}}[5m])) / sum(rate({service}_http_requests_total[5m])) * 100',
                    "legend": "Error Rate",
                }
            ],
            unit="percent",
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'sum(rate(process_cpu_seconds_total{{job="{service}"}}[5m])) * 100',
                    "legend": "CPU %",
                }
            ],
            unit="percent",
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'process_resident_memory_bytes{{job="{service}"}}',
                    "legend": "Memory",
                }
            ],
            unit="bytes",
    ...
    def generate_kubernetes_dashboard(self, namespace: str):
        ...
        self.add_stat(
            "Pending Pods",
            f'count(kube_pod_status_phase{{namespace="{namespace}", phase="Pending"}})',
            width=6,
        )
        self.add_stat(
            "Failed Pods",
            f'count(kube_pod_status_phase{{namespace="{namespace}", phase="Failed"}})',
            width=6,
        )
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'sum(container_memory_usage_bytes{{namespace="{namespace}"}}) by (pod)',
                    "legend": "{{pod}}",
                }
            ],
            unit="bytes",
    ...
    def _generate_postgres_dashboard(self, instance: str):
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'pg_stat_database_numbackends{{instance="{instance}"}}',
                    "legend": "{{datname}}",
                }
            ],
            unit="short",
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'rate(pg_stat_database_xact_commit{{instance="{instance}"}}[5m])',
                    "legend": "Commits",
                },
                {
                    "query": f'rate(pg_stat_database_xact_rollback{{instance="{instance}"}}[5m])',
                    "legend": "Rollbacks",
                },
            ],
            unit="tps",
        ...
        self.add_graph(
            ...
            [
                {
                    "query": f'histogram_quantile(0.95, rate(pg_stat_statements_total_time_bucket{{instance="{instance}"}}[5m]))',
                    "legend": "p95",
                }
            ],
            unit="ms",
    ...
    def _generate_mysql_dashboard(self, instance: str):
        ...
        self.add_graph(
            ...
            [
                ...
                {
                    "query": f'mysql_global_status_threads_running{{instance="{instance}"}}',
                    "legend": "Running",
                },
            ],
            unit="short",
...
Examples:
  ...
  python3 dashboard_generator.py database \\
    ...
    if generator.save(args.output):
        ...
        print("   3. Select datasource and save")
    else:
        sys.exit(1)
