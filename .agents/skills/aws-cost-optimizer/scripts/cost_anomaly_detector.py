...
class CostAnomalyDetector:
    ...
    def print_report(self, forecast_amount: float = None):
        ...
        if self.findings["trend_analysis"]:
            ...
            print(
                tabulate(
                    self.findings["trend_analysis"][:15],
                    headers="keys",
                    tablefmt="grid",
                )
            )
...
Note: This script requires Cost Explorer API access, which may incur small charges.
    ...
    try:
        detector = CostAnomalyDetector(profile=args.profile, days=args.days)
        detector.run()
    except Exception as e:
        ...
        print(
            "2. IAM user has 'ce:GetCostAndUsage' and 'ce:GetCostForecast' permissions",
            file=sys.stderr,
        )
        sys.exit(1)
