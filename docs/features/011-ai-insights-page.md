...
| 2026-05-04 | Status flipped Draft → Implemented. Master-plan Stage 8 landed: proxy.ts matcher + middleware `isProtectedRoute` rename for `/insights`; `apps/web/lib/api/forecast.{ts,types.ts,schema.json}` with hand-written types + JSON-Schema drift CI test; `useWarmForecast` hook with bounded 1500ms race + outcome telemetry to `POST /api/v1/metrics/client-event`; legacy `client.ts::ForecastResponse` renamed `LegacyForecastResponse`; `/insights` page (all-client) + 9 components (BalanceForecastChart recharts fan + SafeToSpendCard + MonthEndSnapshot + OverdraftRiskBadge + ConfidenceBadge + PrimaryDrivers + ScenarioImpactCard + AddPlanModal framer-motion + ColdStartBanner) + WarmTrigger + skeleton + error boundary; vitest tests for component states + warm-timeout-fallback test; Playwright + axe-core e2e at `apps/web/e2e/insights.spec.ts` (skips when no auth fixture). |
