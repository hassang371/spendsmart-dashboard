...
| 2026-05-04 | Status flipped Draft → Implemented. Master-plan Stage 6 landed: `user_intents` + `scheduled_cashflows.source_rule_id` + `upsert_intent_with_bridge` migrations; intent + scenario Pydantic schemas; `intent_bridge.py` (confidence-weighted projection); `IntentsService` CRUD with bridge orchestration; 6 routes (intents CRUD + scenario, rate-limited 20/min CRUD, 5/min scenario); `ForecastService.scenario_predict` parallel A/B forecast via `asyncio.gather` with field-by-field `ScenarioDelta`; LIFE_EVENT + low/medium-confidence intent propagation into `compute_insights(active_intents=...)`. Cascade contract test skip-stubbed pending supabase apply. |
