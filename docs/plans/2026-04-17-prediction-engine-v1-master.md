...
**Master status:** `Implemented` (Stage 9 first-walk-forward run + Stage 10 production-style smoke tests deferred to a separate session — both gated on populated supabase + the pre-existing `public.transactions` schema-bootstrap migration gap that prevents `supabase db reset` from succeeding locally). `Verified` after Stage 9 walk-forward documented + production rollout per the Deployment Gate.
...
- [ ] Write migration `supabase/migrations/20260418000000_user_predictions.sql` per RFC-003 §4 DDL exactly. Includes the `generated_hour timestamptz GENERATED ALWAYS AS date_trunc('hour', generated_at) STORED` column (Codex Fix #1), `UNIQUE (user_id, generated_hour)` index `uniq_user_predictions_user_hour`, lease columns `claimed_at` + `lease_expires_at` (Codex Fix #2), partial index `idx_user_predictions_unevaluated`, RLS, and the `log_user_prediction(payload jsonb)` SECURITY DEFINER RPC.
- [ ] Apply migration locally via `supabase db reset` (or equivalent); verify via `\d public.user_predictions` + `pg_policies` query + `\df log_user_prediction`.
...
- [ ] Write migration `supabase/migrations/20260418200000_scheduled_cashflows.sql` per RFC-005 §Data Model Changes. **Expanded `uniq_scheduled_cashflows_rule` per Codex Fix #3** — includes `(user_id, COALESCE(merchant,''), amount, category_bucket, rrule_freq, COALESCE(day_of_month,-1), COALESCE(day_of_week,-1), source)`. Run pre-apply audit query (RFC-005 §Data Model Changes) to confirm no existing collisions before promoting to production (v1 ships against empty table; audit is precautionary).
...
- [ ] Wire `ForecastService.predict`: tier routing + insights + `uuid4()` prediction_id + atomic dedup via `supabase.rpc("log_user_prediction", {...})` (Codex Fix #1 — no SELECT-then-INSERT). Wrap `compute_insights` in guard with fallback to `_safe_default_insights`. Use user-scoped Supabase client (per RFC-003 §4 INSERT RLS policy `users insert own predictions` + RPC SECURITY DEFINER).
- [ ] Add concurrency test `test_service_logging.py::test_concurrent_predict_does_not_duplicate` per RFC-003 §3b: fires N parallel `predict()` calls under `asyncio.gather`; asserts exactly one row in `user_predictions` for the (user_id, hour) pair.
...
- [ ] Create `apps/api/core/tasks/evaluate_predictions.py` — Celery beat task `evaluate_past_predictions`. Lease-based two-pass claim per RFC-003 §5 (Codex Fix #2): pass 1 = atomic `UPDATE ... FROM claimable ... RETURNING` setting `claimed_at = now(), lease_expires_at = now() + interval '15 minutes'` (NOT `evaluated_at`); pass 2 = per-row UPDATE setting `actual_outcomes`, `mape`, `pinball_loss`, `evaluated_at = now()` only after metrics successfully computed. Claim predicate is `evaluated_at IS NULL AND (claimed_at IS NULL OR lease_expires_at < now())`. Pinball-loss impl validated against `sklearn.metrics.mean_pinball_loss` in `test_evaluate_predictions.py`. Add invariant test `test_crashed_worker_row_is_reclaimable` seeding a row with stale lease and asserting re-claim succeeds.
...
- [ ] Create 9 components under `apps/web/app/insights/components/` per LLD 011 §Component Specs: BalanceForecastChart (recharts fan chart), SafeToSpendCard, MonthEndSnapshot, OverdraftRiskBadge, ConfidenceBadge, PrimaryDrivers, ScenarioImpactCard, AddPlanModal (framer-motion, not shadcn), ColdStartBanner, WarmTrigger.
- [ ] Implement `useWarmForecast()` hook per Codex Fix #4 — bounded 1500ms wait racing warm against predict, outcome telemetry posted to `POST /api/v1/metrics/client-event`. Page mount fires warm + predict in parallel; predict proceeds regardless of warm outcome. Module-level `inflight` + `lastFiredAt` sentinels prevent duplicate fires.
- [ ] Add test `WarmTrigger.test.tsx::test_predict_proceeds_when_warm_times_out` per Codex Fix #4 — mock `warmForecast` to never resolve; assert page still renders forecast within acceptable time.
...
- [ ] Verify `GET /metrics/prom` returns Prometheus exposition format with all **eleven** RFC-004 metrics enumerated by name: `tft_cache_hits_total`, `tft_cache_misses_total`, `tft_cache_evictions_total{reason}`, `tft_cache_load_duration_seconds`, `tft_cache_resident_entries`, `tft_cache_resident_bytes`, `tft_cache_pubsub_invalidations_total`, `tft_cache_pubsub_invalidations_skipped_stale_total`, `tft_cache_pubsub_publish_failures_total`, `tft_cache_subscriber_reconnects_total`, `forecast_warm_outcome_total{result}`. Stage fails if any metric is missing.
...
| 2026-04-17 | Codex adversarial review fixes propagated. **Stage 2 (Codex Fix #1)** migration adds `generated_hour` GENERATED column + `UNIQUE (user_id, generated_hour)` + `log_user_prediction` SECURITY DEFINER RPC. **Stage 2 (Codex Fix #2)** migration adds `claimed_at` + `lease_expires_at` lease columns. **Stage 4 (Codex Fix #3)** `uniq_scheduled_cashflows_rule` expanded to include category_bucket + day_of_month + day_of_week + source. **Stage 3 (Codex Fix #4)** Prometheus subsystem grows to ELEVEN metrics (adds `forecast_warm_outcome_total{result}`); new `POST /api/v1/metrics/client-event` route added. **Stage 5** service uses RPC for atomic insert-or-no-op, no SELECT-then-INSERT; lease-based two-pass evaluate task; new concurrency test + invariant test. **Stage 8** `useWarmForecast()` hook with 1500ms race + telemetry replaces fire-and-forget WarmTrigger; new timeout-fallback test. |
| 2026-04-17 | Codex pass-2 fixes. Stage 10 metric-count gate enumerates all eleven metrics by name (no more "nine"). RFC-003 RPC param canonicalised to `payload`; trust boundary hardened — server derives `generated_at`/`horizon_end`, validates `horizon_days` 1..30, validates `model_type` enum, requires NOT NULL on prediction_id/insights_version/forecast/insights. New required test `test_log_user_prediction_rpc_hardening`. RFC-005 stale risk row updated to expanded key; persistence-schema paragraph synced. LLD 011 stripped legacy fire-and-forget refs; useWarmForecast tracks explicit outcome state. |
| 2026-05-04 | Execution. Stages 0–8 + Stage 7 + partial Stage 10 landed across 27 commits on `feature/prediction-engine-v1`. Final `make check` green: 590 pytest passed + 6 skipped (5 await populated supabase) + 0 lint errors + tsc clean + 179 vitest passed. Doc statuses flipped: LLD 009/010/011 Draft→Implemented; RFC-003/004/005 Proposed→Implemented; RFC-006 Proposed→Implemented (code-complete). BUG-018: Fix Applied (Stage 3). Aggregate DEVIATIONS recorded in per-doc changelogs. **Deferred work:** Stage 9 walk-forward run + research doc 002 (needs populated supabase + real `train_predict`/`fetch_actuals` wiring + ProcessPoolExecutor parallelism); Stage 10 smoke (`supabase db reset` blocked on missing `CREATE TABLE public.transactions` migration — pre-existing repo gap, not introduced by this plan); production deployment per Deployment Gate. |
