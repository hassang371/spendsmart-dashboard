...
| `scheduled_cashflows` unique index (user_id, merchant, amount, rrule_freq) blocks legitimate duplicate rules | **Very low.** Two rent payments at different amounts are handled (different amount). Two identical patterns are duplicates by design. | Upsert semantics — ON CONFLICT DO UPDATE overwrites the older rule. No manual deduplication needed. |
