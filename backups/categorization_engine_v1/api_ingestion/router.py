...
async def import_file(
    ...
    try:
        ...
        if (
            ...
            or "not a zip file" in err_lower
            or "decryption" in err_lower
        ):
            raise HTTPException(
                status_code=400,
    ...
    for i in range(0, len(insert_rows), BATCH_UPSERT_SIZE):
        ...
        try:
            ...
            if result.data:
                ...
                inserted_ids.extend(r["id"] for r in result.data if r.get("id"))
            else:
                inserted += len(batch)
        except Exception as e:
            logger.error(
            ...
            for row in batch:
                try:
                    r = client.table("transactions").insert(row).execute()
                    inserted += 1
    ...
    # --- Track classification job (after insert so we have real UUIDs) ---
