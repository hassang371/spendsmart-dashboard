...
class TrainingMonitor:
    ...
    def log_batch(
        ...
        if batch % self.log_frequency == 0:
            logger.info(f"Epoch [{epoch}] Batch [{batch}/{total_batches}] " f"Loss: {loss:.4f} LR: {learning_rate:.6f}")
    ...
    def should_stop_early(self, patience: int = 10, metric: str = "val_accuracy") -> bool:
        """Check if training should stop early."""
        if len(self.metrics_history) < patience:
            return False

        # Check if metric has improved in last N epochs
        recent_metrics = [m.get("validation", {}).get(metric, 0) for m in self.metrics_history[-patience:]]
...
class HypCDTrainingPipeline:
    ...
    def __init__(self, config: TrainingConfig):
        ...
        self.trainer = None
        self.checkpoint_manager = CheckpointManager(config.checkpoint_dir, config.keep_last_n)
    ...
    def initialize_model(self, resume_from: Optional[str] = None):
        ...
        if self.config.distributed:
            self.classifier = DDP(self.classifier, device_ids=[self.config.local_rank])
    ...
    def prepare_data(
        self, data_loader_fn: Callable[[], Tuple[List[str], List[int]]]
        ...
        val_loader = None
        if val_texts:
            val_dataset = TransactionDataset(val_texts, val_labels, self.backend, augment=False)
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int) -> Dict:
        ...
        for batch_idx, (embeddings, labels, texts) in enumerate(progress_bar):
            ...
            criterion = HierarchicalLoss(self.config.num_classes, alpha=self.config.lambda_hierarchy)
    ...
    def train(
        ...
        # Save final metrics
        self.monitor.save_metrics(os.path.join(self.config.checkpoint_dir, "metrics_history.json"))
...
def create_default_pipeline(
    checkpoint_dir: str = "checkpoints", epochs: int = 50, distributed: bool = False
