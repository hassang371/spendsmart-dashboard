...
class TestTrainingPipeline:
    ...
    def test_model_initialization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config = TrainingConfig(checkpoint_dir=temp_dir, epochs=1)
            pipeline = HypCDTrainingPipeline(config)
    ...
    def test_training_epoch(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config = TrainingConfig(checkpoint_dir=temp_dir, epochs=1, batch_size=2)
            pipeline = HypCDTrainingPipeline(config)
            pipeline.initialize_model()
...
class TestPipelineIntegration:
    ...
    def test_create_default_pipeline(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            pipeline = create_default_pipeline(checkpoint_dir=temp_dir, epochs=10)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        class TinyDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, i):
                return embs[i], labels[i], texts[i]
