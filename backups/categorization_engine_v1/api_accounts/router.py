...
async def get_transaction_counts(client: Client = Depends(get_user_client)):
...
async def batch_update_transactions(
    ...
    for (category, amount), ids in groups.items():
        try:
            ...
            result = client.table("transactions").update(payload).eq("user_id", user_id).in_("id", ids).execute()
            if result.data:
