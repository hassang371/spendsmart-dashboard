...
def ingest_file(file_path: str, user_id: str):
    ...
    # Single statement upsert avoids partial-apply semantics across batches.
    try:
        supabase.table("transactions").upsert(
            records,
            on_conflict="user_id,fingerprint",
        ).execute()
        print(f"✅ Upsert successful for {len(records)} records")
    except Exception as e:
        print(f"❌ Import failed: {e}")
        raise RuntimeError("Import failed; no guarantees for write durability.") from e
